...
class ModelManager(QObject):
    ...
    def load_custom_model(self, config_file):
        ...
        if (
            ...
            or model_config["type"] not in ["segment_anything", "yolov5", "yolov8", "yolov11"]
        ):
            self.new_model_status.emit(
    ...
    def _generate_config_from_pt(self, pt_file):
        ...
        try:
            ...
            logging.error("ultralytics not installed. Cannot auto-generate config from .pt file")
            return None
        ...
        try:
            ...
            if hasattr(model, 'task'):
                ...
                model_name = str(model.ckpt.get('model', '')).lower() if hasattr(model, 'ckpt') else ''
            ...
            self.new_model_status.emit(
                self.tr(f"Config file generated: {config_file}")
            )
