        ...
        try:
            ...
            for idx, image_path in enumerate(self.image_paths):
                ...
                try:
                        ...
                        for shape in result.shapes:
                            ...
                            # Also check shape description for confidence
                                ...
                                try:
                                    ...
                                    if isinstance(shape.description, str) and '%' in shape.description:
                    ...
                    # Emit progress
                    self.progress.emit(idx + 1, len(self.image_paths), len(filtered_images))
...
class ImageFilterDialog(QDialog):
    ...
    def save_settings(self):
        ...
        config["image_filter"]["min_confidence"] = self.confidence_slider.value() / 100.0
    ...
    def populate_model_combo(self):
        ...
        if self.model_manager.loaded_model_config:
            model_name = self.model_manager.loaded_model_config.get("display_name", "Current Model")
            self.model_combo.addItem(f"{model_name} (loaded)", self.model_manager.loaded_model_config)
        else:
    ...
    def on_progress(self, current, total, matched):
        ...
        if total > 0:
            ...
            self.matched_label.setText(
                self.tr(f"Matched: {matched} images")
            )
    ...
    def on_filtering_finished(self, filtered_images):
        ...
        QMessageBox.information(
            ...
            self.tr(f"Found {len(filtered_images)} images with detections out of {len(self.image_paths)} total images.")
        )
